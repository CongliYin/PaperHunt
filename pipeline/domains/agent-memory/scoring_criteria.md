...
Security-first work is always excluded, including attacks, poisoning, privacy leakage, cryptographic protection, prompt injection, and forensic detection. Give it `domain_fit <= 0.2`. Medical, clinical, and surgical applications are excluded. Research on false-memory promotion, conflict resolution, consolidation correctness, provenance, or verifier-backed updates remains eligible when its main objective is memory quality rather than security defense.

Do not misclassify governance requirements inside a general-purpose Agent-memory service as security-first research. Permission scoping, tenant isolation, provenance, leakage prevention, auditability, and propagation policy remain in scope when they are supporting primitives or evaluation dimensions of a reusable shared-memory architecture. A production shared-memory service centered on scoped retrieval, temporal supersession, provenance tracking, contradiction handling, and controlled propagation should normally receive `domain_fit >= 0.80`, even if its evaluation reports authorization or cross-tenant leakage failures. Exclude it only when the paper's primary contribution is an attack, defense, privacy mechanism, or security benchmark rather than memory construction and lifecycle management.
