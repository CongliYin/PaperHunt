...
Security-focused and medical/clinical/surgical papers are globally out of scope. Give them `domain_fit <= 0.2` even when they claim realtime understanding, low latency, streaming video, Agent use, or intraoperative decision support. Low-latency perception alone is not interactive agency.
...
3. **streaming_video_grounding** (0-1): Does the work support realtime video understanding, scene localization, visual grounding, visual evidence, video memory, visual shopping, or visual tool calls? Pure image/video classification, generic VQA, and medical/surgical video understanding should score low.
...
- Pure video understanding papers score high only when they handle streaming constraints, visual grounding for actions, video memory, or live non-medical assistance workflows.
- Visual grounding papers score high when grounding triggers or verifies tool actions, product/device identification, recommendation, or proactive alerts outside medical/clinical settings.
